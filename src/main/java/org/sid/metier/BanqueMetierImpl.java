...
import org.sid.dao.IAgenceRepository;
  ...
  // pour faire l'injection de dependance  --> on va demander a spring d'injecter une implementation de cette interface
	...
	public Client getClient(Long codeClient) {
		Client client=clientRepository.findOne(codeClient);
		 if (client==null) throw new RuntimeException("Client introuvable"); // c'est une exception non surveiller
		return client;
	}

	@Override
	public Page<Client> listClientsAgences(Long Code, int page, int sizePage) {
		return clientRepository.listClient(Code, new PageRequest(page,sizePage) );
	}
	/*  @Override
	public List<Client> listClientsAgences(Long code) {  // page: est le numero de la page
		 
		return clientRepository.listClient(code);
	} */
